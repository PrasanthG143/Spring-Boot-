	...
	protected void configure(HttpSecurity http) throws Exception {
      //  http.csrf().disable().authorizeRequests().anyRequest().permitAll(); // Works for GET, POST, PUT, DELETE
       http .authorizeRequests().antMatchers("/", "/list")
		  .access("hasRole('USER') or hasRole('ADMIN') or hasRole('DBA') or hasRole('MANAGER') or hasRole('EMPLOYEE')"
		  ) .antMatchers("/usermanagement/**")
		  .access("hasRole('ADMIN') or hasRole('DBA')")
		  .antMatchers("/billupload/**").access("hasRole('ADMIN') or hasRole('DBA') or hasRole('MANAGER') or hasRole('EMPLOYEE')")
